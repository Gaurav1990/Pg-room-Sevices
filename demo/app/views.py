...
from app.models import Pg , Ad , UserProfile , Rating , Reviews , EmailNotification
...
def home(request):
    ...
    if rating_avg is None:
        rating_avg = 0.0
    try:
        email_noti = EmailNotification.objects.filter(user = request.user)[0]
    except:
        email_noti = None    
...
@csrf_exempt
def paste(request):
    email_notification = EmailNotification.objects.all()
    #print "1"
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for item in email_notification:
                if user.area == item.place and item.is_require == True:
                    tt = loader.get_template('email.txt')
                    c = Context({'user':user, 'item.user.username' : item.user.username})
                    
                    e = EmailMultiAlternatives('Email Notification about pg', tt.render(c), 'pgroomservices@gmail.com', [item.user.email])
                    
                    try:
                        ht = loader.get_template('notification_email.html')
...
def send_pass(request):
    ...
    if data_dict == [] and email_id == [] :
        state = "This user name is not exist in database. Please check again"
        return render_to_response('forgot_user.html',locals())  
...
def password_change(request):
    ...
    if pass1 != pass2 or pass1 == '':
        state = "New Passwords are not matched or empty .Please enter again"
        return render_to_response('change.html',locals())
    user_data = User.objects.all()
    ...
    for item in user_data:
        ...
        if request.user.username == item.username:
            if not item.check_password(oldPassword):
                state = "Your old password is wrong. Please fill correct"
                return render_to_response('change.html',locals())
            
            tt = loader.get_template('email.txt')
            c = Context({'oldPassword':oldPassword , 'pass1' : pass1})
    
            e = EmailMultiAlternatives('Reset-Password', tt.render(c), 'pgroomservices@gmail.com', [item.email])
    
            try:
                ht = loader.get_template('reset_pass.html')
                e.attach_alternative(ht.render(c), 'text/html')
            except:
                pass
            up = UserProfile.objects.get( password2 = oldPassword )
            up.password2 = pass1            
            up.save()
            ...
            name = item.username
            e.send()
            item.set_password(str(pass1))
            item.save()
    return render_to_response('change_pass_done.html',locals())

def notification(request):
    query = Ad.objects.all().query
    query.group_by = ['area']
    ads = QuerySet(query = query, model = Ad)
    return render_to_response('notific.html',locals())
...
def noti(request):
    ...
    #ITPL = request.POST.get("ITPL")
    query = Ad.objects.all().query
    query.group_by = ['area']
    a = QuerySet(query = query, model = Ad)
    #print request.user.username
    for item in a:
        #print str(item.area)
        if request.POST.get(item.area) == "on":
            email_noti = EmailNotification(user = request.user , is_require = True, place = item.area)
            #print email_noti.user            
            #print email_noti.is_require
            #print email_noti.place
        else:
            email_noti = EmailNotification(user = request.user, is_require = False, place = item.area)
            #print email_noti.user
            #print email_noti.is_require
            #print email_noti.place
    
        email_noti.save()
    email_all = EmailNotification.objects.all()
    return render_to_response('data_saved_notification.html',locals())

@csrf_exempt
def notification_all(request):
    email_noti = EmailNotification.objects.all()
    return render_to_response('admin_noti.html',locals())

def delete_noti_admin(request,d_id):
    email_noti = EmailNotification.objects.get(id = d_id).delete()
    email_noti = EmailNotification.objects.all()
    return render_to_response('admin_noti.html',locals())

def remove_noti(request):
    email_noti = EmailNotification.objects.filter( user= request.user).delete()
    return HttpResponseRedirect(reverse('app.views.home'))
